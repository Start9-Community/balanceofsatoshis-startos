...
- **Hard dependency on LND.** BoS mounts LND's `main` volume read-only at `/mnt/lnd` to read `tls.cert` and the admin macaroon, and reaches LND's gRPC over the LXC bridge — `main.ts` resolves LND's gRPC bridge address through `sdk.host.getBridgeAddress` (mapping `gRPCHostId`/`gRPCPort` imported from `lnd-startos/startos/interfaces` to `10.0.3.1:<assigned port>`) and writes the resolved `host:port` into the saved-node `credentials.json`. The helper is chained with `.const()`, so it resolves the binding’s own derived bridge address and `main` restarts only when that address actually changes — not on LND updates or lock/unlock cycles. LND binds gRPC only after its wallet is first unlocked; until then (and while LND is absent) the address resolves null and `socket` is left unset (`z.string().optional().catch(undefined)`) rather than pointed at an unreachable placeholder, so `bos peers` reports not-yet-ready and `main` heals with one restart when LND's gRPC appears. LND's StartOS-issued cert covers its bridge address, so the pinned connection still verifies.
